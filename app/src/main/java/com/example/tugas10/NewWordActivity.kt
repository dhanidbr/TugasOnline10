...
class NewWordActivity : AppCompatActivity() {
    companion object {
        const val EXTRA_ID = "com.example.tugas10.EXTRA_ID"
        const val EXTRA_TITLE = "com.example.tugas10.EXTRA_TITLE"
        const val EXTRA_DESCRIPTION = "com.example.tugas10.EXTRA_DESCRIPTION"
        const val EXTRA_PRIORITY = "com.example.tugas10.EXTRA_PRIORITY"
    }
    ...
    private fun saveWord() {
        if (edit_text_title.text.toString().trim().isBlank() || edit_text_description.text.toString().trim().isBlank()) {
            Toast.makeText(this, "Can not insert empty word!", Toast.LENGTH_SHORT).show()
            return
        }
